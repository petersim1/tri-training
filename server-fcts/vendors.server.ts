...
import type { HevyRoutineSummary } from "@/lib/hevy/types";
...
export const startStravaOAuth = createServerFn({ method: "POST" }).handler(
...
export const persistServiceStravaTokens = createServerFn({ method: "POST" })
...
export const getValidAccessTokenForWebhooks = createServerFn({
  method: "GET",
}).handler(async () => {
  const db = await getDb();
  const row = await db
