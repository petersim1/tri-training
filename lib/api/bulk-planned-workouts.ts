...
import type { NewPlannedWorkout } from "../db/schema.server";
...
export function parseBulkPlannedWorkoutsBody(
  ...
  for (let i = 0; i < rawList.length; i++) {
    ...
    if (duP.value !== null) {
      const u = duP.value.trim().toLowerCase();
      if (u !== "") {
        ...
        distanceUnits = u as CardioDistanceUnit;
      }
    }
...
export const validateAndBuildRow = (
  ...
  index: number,
  now: Date,
): NewPlannedWorkout | BulkValidationIssue => {
  try {
