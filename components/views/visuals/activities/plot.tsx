...
import { VALID_CUMULATIVE } from "@/lib/constants/visuals";
...
const formatValue = (v: number, metric: SessionChartMetric) => {
  if (metric === "volume")
...
export const createViz = (
  ...
  const enriched: EnrichedPoint[] = sorted.map((p, i) => ({
    ...p,
  ...
  // Data
  if (cumulative && cumulativeOk && enriched.length >= 2) {
    svg
      .append("path")
      .datum(enriched)
      .attr("fill", "none")
