...
const completedWorkoutOpenInVendor = (
  ...
  return {
    href: isStrava ? stravaActivityWebUrl(cw.vendorId) : hevyWorkoutWebUrl(cw.vendorId),
...
export const ActivityElement: React.FC<{
  ...
  const title = workout.vendorActivity ? completedWorkoutTitle(workout.vendorActivity) : null;
  const brief = workout.vendorActivity ? formatCompletedSessionBrief(workout.vendorActivity) : null;
  ...
  return (
    <>
      <div
        ...
        className={cn(
          "min-w-0",
          isCard && "rounded-md border border-zinc-800/80 bg-zinc-950/70 px-2 py-2",
        )}
      >
        <div className="flex min-w-0 flex-col gap-1">
          ...
          {!hideDate && (
            <p className="text-[9px] tabular-nums text-zinc-500" title={workout.dayKey}>
          ...
          {workout.vendorActivity ? (
            ...
            <div className="mt-2 rounded border border-zinc-800/60 bg-zinc-900/30 px-1.5 py-1 h-14 flex flex-col justify-center gap-0.5 pl-1.5">
              {planTargets && (
                <p className="text-[10px] font-medium text-zinc-300">{planTargets}</p>
              )}
