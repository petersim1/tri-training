import type { HevyRoutineSummary, HevyWorkout } from "./types";
...
export const hevyFetchRoutineById = async (
  routineId: string,
): Promise<HevyRoutineSummary | null> => {
  const id = routineId.trim();
  if (!id) {
    return null;
  }
  const url = `${BASE}/routines/${encodeURIComponent(id)}`;
  const res = await fetch(url, {
    headers: {
      ...headers(),
      Accept: "application/json",
  ...
  return data.routine as HevyRoutineSummary;
};
