...
export const persistTurn = async (
  ...
  const messages = await db.insert(chatMessages).values(messagesStore).returning();

  const assistantMessage = messages.find((m) => m.role === "assistant") as ChatMessageItem | null;
  const userMessageRow = messages.find((m) => m.role === "user") as ChatMessageItem | null;
