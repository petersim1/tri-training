...
const tracer = trace.getTracer("bevor.backfill");

const backfillLinkedWorkouts = createServerFn({
  method: "POST",
}).handler(async (): Promise<BackfillReport> => {
  return tracer.startActiveSpan("backfillLinkedWorkouts", async (span) => {
    try {
