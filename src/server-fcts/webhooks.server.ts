...
const logWebhookDelivery = async (data: {
...
const processHevyWebhook = async (
  ...
  const w = await hevyFetchWorkoutById(workoutId);
  const db = await getDb();
  const now = new Date();

  if (!w) {
    await deleteCompletedByVendor("hevy", workoutId);
    return {
  ...
  const existing_workout = await db
    ...
    .where(eq(vendorActivities.id, workoutId))
    .get();
  if (existing_workout) {
    await db
...
const processStravaWebhook = async (
